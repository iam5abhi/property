...
export default function AddQuery({setOpen,open,getQueriesData}) {
  ...
  const handleSubmit =(event)=>{
      ...
      body: JSON.stringify({...formData,ProjectID:projectData[0].ProjectID,requirement:projectData[0].requirement,ProjectName:projectData[0].ProjectName,budget:projectData[0].PriceStartsfrom,type:projectData[0].type}),
      }).then((res) => {
        getQueriesData() 
        setOpen(false)
      })
                        ...
                        <form onSubmit={handleSubmit}>
                              <div className="mb-4">
                                ...
                                <select id="AvailableFrom" name="ProjectID" onChange={handleChange}
                                  ...
                                  {projects.map((data)=><option key={data.id} value={data.id}>{data.ProjectName}({data.requirement})({data.PriceStartsfrom})({data.type})</option>)}
